...
def load(path: Path) -> LoadedModel:
    ...
    try:
    ...
    except (OSError, EOFError, ValueError, KeyError, pickle.UnpicklingError) as exc:
        ...
        # refusing its contents — `joblib` raises `KeyError` on a bad magic
        # number, which is why that class is here. Anything else is a defect and
