...
**Should say:** for each event, one sentence in the emitting module's contract plus a §3.2 case. `logging_setup` must add `moscow_time` on every JSON record (UTC `timestamp` already exists). The amendment must settle whether `event` is **required on every record** (formatter default) or **producer-set and optional** — §7.1 currently says the former; a proposal must not pick the latter in passing. Do not add a second public logger API unless the spec names it.
