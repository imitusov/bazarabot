...
**V13 — `verify_trading_status.py`.** On the live account, read-only, for every
ticker in `WATCHLIST`: retrieves the instrument's trading status from the
market-data service **on its own**, without `share_by`. PASS requires, for every
ticker without exception: the endpoint exists on the pinned SDK and resolves the
ticker's FIGI; the status it returns is a `SECURITY_TRADING_STATUS_` name, so the
string `broker.client` already derives from `share_by` is the same string derived
from this source for the same instrument in the same minute; and the request
sits in the market-data budget with at least 2× headroom over one call per
watchlist ticker per poll, measured the way V7 measures the candle endpoint.
Prints the observed status per ticker and the measured headroom. FAIL if the
endpoint is absent, if the two sources disagree for any ticker, or if headroom is
under 2×.

**V13 gates one §4 branch and is not in `run_all.sh` (v1.85).** The suite is the
pre-development gate for the project; this check is the pre-development gate for
`broker.client`'s instruments cache, whose cache-hit branch reads the trading
status from this endpoint and must not be written against a mock that agrees with
...
| Operations feed, **`quantity` unit** | **Instrument units, not lots.** A sale of AFLT at 6 lots × lot size 10 reports `quantity=60` | Measured 2026-09-18 by V12 against the live account, cross-checked against the closed position that produced it. The V12 docstring recorded this as undecidable and warned against "fixing" the parse on a guess; it is now decided. `_resolve_sale` divides gross by units so it was never wrong either way, but an agent reading `quantity` as lots would size a remedy 10× small on any instrument whose lot is not 1 |
| Operations feed, SELL rows | **9 sell rows over 90 days** — `_resolve_sale`'s input exists | Measured 2026-09-18 by V12, which reached **6/6** for the first time. From the day it was written until #44 this check failed by design: the account had never sold, so the function `broker.reconcile` books every external close from had no production evidence behind it |
| Operations feed, types seen | `OPERATION_TYPE_BUY`, `OPERATION_TYPE_BROKER_FEE`, `OPERATION_TYPE_INPUT`. No SELL row existed until 2026-09-18 (now 9; see the rows above) | Measured 2026-09-10 by V12. `broker.reconcile._resolve_sale` is therefore **unmeasured**: the path that books an external close has never run against real data. V12's SELL check fails by design until the account has sold once (#44) |
...
**The sentinel is per field, and assuming otherwise is its own defect (v1.81).**
A populated-looking timestamp is not evidence that the field is set — that is the
paragraph above — but the converse does not follow either: that two fields of a
message hold the sentinel says nothing about a third. `TradingDay` on a closed
day holds the sentinel in `start_time` and `end_time` and a real date in `date`,
measured above. Whether a given field is set is settled by reading it against the
account, one field at a time, and never by inference from its neighbours.
...
section by heading: a line whose whole content is `**text**` opens a block and
closes the one before it, and only a heading named in the generator's `M` table
reaches a task file. A sub-group therefore never gets a bare bold heading of its
own — it carries trailing prose naming the module that owes it, the way the
stop-order lifecycle and partial-fill groups below name `execution.orders`,
which keeps its cases inside the owning module's block. A bare heading that no
`M` row names is a group of cases written for nobody: eleven `execution.orders`
partial-fill cases sat unreachable that way until v1.80 (#189).
...
- `006_trading_days.sql` creates `trading_days`. It holds no history at first,
  ...
  *from*, since the broker will not serve a past schedule at all. The table fills
  from the first `refresh` onwards, fourteen days at a time. **v1.81 adds no
  migration**: `session_start` and `session_end` were declared `TEXT NULL` in
  `006` with the note "Null on a non-trading day", so recording closed days
