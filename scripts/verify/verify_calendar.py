...
async def body():
    ...
    async with client(TOKEN) as c:
        ...
        response = await c.instruments.trading_schedules(
            ...
            from_=day_start,
            to=day_start + dt.timedelta(days=14),
        )

        exchanges = list(response.exchanges)
        v.check(
            "trading schedules are retrievable",
            bool(exchanges),
        ...
        if not v.check(
            "an exchange matching '{}' is present".format(EXCHANGE_HINT),
            bool(matches),
            "available: {}".format(
        ...
        v.check(
            "trading and non-trading days are distinguished",
        ...
        if trading:
            day = trading[0]
            aware = (
                day.start_time.tzinfo is not None and day.end_time.tzinfo is not None
            )
            v.check(
                "session times are timezone-aware",
                aware,
                "{} .. {}".format(day.start_time, day.end_time),
            )
            msk = dt.timezone(dt.timedelta(hours=3))
            v.note(
                "next session in MSK: {} .. {}".format(
