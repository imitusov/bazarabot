...
M = [
 (1,"models","zarabot/models.py",None,[],[22],"Shared domain types. Every other module's signatures are written in these types, so this is the vocabulary the whole system speaks."),
 ...
 (3,"config","zarabot/config.py",None,[],[15],"Loads and validates every setting once at startup. The bot refuses to start rather than trade on an assumed risk limit."),
 ...
 (6,"db.positions","zarabot/db/positions.py","`db.positions`",["positions"],[11,12],"Sole owner of position rows. Positions are never deleted; closing is a state transition, because the history is the point of the project."),
 (7,"db.orders","zarabot/db/orders.py","`db.orders`",["orders"],[5,11,12],"Sole owner of order rows. Records intent BEFORE the broker is called, which is what makes a crash mid-submission recoverable."),
 (8,"db.stop_orders","zarabot/db/stop_orders.py",None,["stop_orders"],[11,12],"Sole owner of stop-order rows. Tracks the standing stop the exchange holds for each open position."),
 (9,"db.cooldowns","zarabot/db/cooldowns.py","`db.cooldowns`",["cooldowns"],[12],"Sole owner of per-instrument re-entry cooldowns, which replace a daily order cap as the runaway-loop protection."),
 (10,"db.signals","zarabot/db/signals.py","`db.signals` / `db.snapshots`",["signals"],[12],"Records every signal with its risk decision, approved or rejected. Rejections are analysed in the weekly report."),
 (11,"db.snapshots","zarabot/db/snapshots.py","`db.signals` / `db.snapshots`",["daily_snapshots"],[12],"Daily equity snapshots. The opening baseline is what the daily loss limit measures against."),
 ...
 (21,"broker.client","zarabot/broker/client.py","`broker.client`",[],[1,2,3,4,5,19,28],"The ONLY module that talks to the broker. Wraps t_tech.invest.AsyncClient and returns domain types. Run the verification suite before building this."),
 ...
 (24,"state.halt","zarabot/state/halt.py","`state.halt`",["halt_state"],[20],"Sole owner of the halt flag. A halt suspends ENTRIES ONLY - exits keep running, and the halt survives restarts."),
 ...
 (26,"execution.orders","zarabot/execution/orders.py","`execution.orders`",["positions","orders","stop_orders","cooldowns"],[3,4,5,11,23,26,27,28],"Where money moves. Write-then-send ordering, the submission locks, crash recovery, and the standing stop-loss. Highest-risk module in the project. 95% coverage."),
 (27,"broker.reconcile","zarabot/broker/reconcile.py","`broker.reconcile`",["positions","stop_orders","reconciliations"],[6,7,24,25],"Compares broker truth against local belief on startup. Observes and reports only - it never places or cancels an order."),
