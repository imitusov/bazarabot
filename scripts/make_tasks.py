...
M = [
    ...
    (
        ...
        [],
        [],
        "The Strategy protocol. Pure, entry-only, deterministic - the properties that let the backtester share this code with the live path.",
    ),
    (
        15,
    ...
    (
        ...
        "`execution.orders`",
        ["positions", "orders", "stop_orders", "cooldowns"],
        [3, 4, 5, 11, 23, 26, 27, 33, 34],
    ...
    (
        ...
        "`broker.reconcile`",
        ["positions", "stop_orders", "reconciliations"],
        [6, 7, 24, 25, 30, 32, 33],
    ...
    (
        ...
        "`reporter.weekly`",
        ["positions", "signals", "daily_snapshots"],
        [12, 13],
...
def rule(n):
    m = re.search(r"^{}\. (\*\*.*?)(?=^\d+\. \*\*|\Z)".format(n), ERRORS, re.M | re.S)
...
for order, name, spec_key, tkey, tables, rules, context in M:
    path = (
        "sandbox/{}.py".format(name.split(".")[-1])
    ...
    contract = section(
        spec_key
    ) or "SPEC SECTION NOT FOUND — read `{}` in technical-spec.md §4 yourself.".format(
        spec_key
    )
    tests = (
        test_block(tkey)
        or "No dedicated test block in §3.2. Derive cases from the contract above: happy path, every early return, every boundary, and every documented exception."
    )
    ...
    body += [
        ...
        "",
        "Module **{}** of {} in `dependency-order.md`. Everything before it is complete and tested — **do not modify any of it**.".format(
            order, len(M)
        ),
        "",
    ]
    ...
    body += [
        "## Agent instructions",
        "",
        "1. Write `{}` FIRST, from the test cases above. No implementation yet.".format(
            testfile
        ),
