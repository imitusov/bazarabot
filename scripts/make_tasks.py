...
M = [
 ...
 (15,"strategies.ma_crossover","zarabot/strategies/ma_crossover.py","`strategies.*`",[],[],"Moving-average crossover entries. Pure function over candles."),
 (16,"strategies.rsi_reversion","zarabot/strategies/rsi_reversion.py","`strategies.*`",[],[],"RSI mean-reversion entries. Pure function over candles."),
 (17,"strategies.momentum","zarabot/strategies/momentum.py","`strategies.*`",[],[],"Momentum breakout entries. Pure function over candles."),
 ("17b","strategies.volume_breakout","zarabot/strategies/volume_breakout.py","`strategies.*`",[],[],"Momentum breakout confirmed by volume. The only strategy that reads Candle.volume, which every fetch carries and nothing read before. Pure function over candles."),
 ("17c","strategies.bollinger_reversion","zarabot/strategies/bollinger_reversion.py","`strategies.*`",[],[],"Mean reversion at the lower Bollinger band. Volatility-scaled where rsi_reversion is a fixed threshold. Decimal square root, never math.sqrt. Pure function over candles."),
 ("17d","strategies.macd_trend","zarabot/strategies/macd_trend.py","`strategies.*`",[],[],"MACD line crossing above its signal line. Trend like ma_crossover, but momentum-of-momentum rather than a level comparison. Pure function over candles."),
 ...
 (19,"strategies.registry","zarabot/strategies/registry.py","`strategies.*`",[],[],"Builds the active strategy set from configuration."),
 ...
 (21,"broker.client","zarabot/broker/client.py","`broker.client`",["instruments"],[1,2,3,4,5,12,19,28,30,31,33],"The ONLY module that talks to the broker. Wraps t_tech.invest.AsyncClient and returns domain types, and from spec v1.85 it is also the sole writer and sole reader of the instruments cache. Run the verification suite before building this."),
 ...
 (24,"state.halt","zarabot/state/halt.py","`state.halt`",["halt_state"],[20,30],"Sole owner of the halt flag and of the halt_state table, which it writes itself - there is no db.halt_state repository. A halt suspends ENTRIES ONLY - exits keep running, and the halt survives restarts."),
 ...
 (26,"execution.orders","zarabot/execution/orders.py","`execution.orders`",["positions","orders","stop_orders","cooldowns"],[3,4,5,11,23,26,27,33,34],"Where money moves. Write-then-send ordering, the submission locks, crash recovery, and the standing stop-loss. Highest-risk module in the project. 95% coverage."),
 (27,"broker.reconcile","zarabot/broker/reconcile.py","`broker.reconcile`",["positions","stop_orders","reconciliations"],[6,7,24,25,30,32,33],"Sole owner of the reconciliations table, which it writes itself - there is no db.reconciliations repository. Compares broker truth against local belief on startup. Observes and reports only - it never places or cancels an order."),
 ...
 (32,"app.startup","zarabot/app/startup.py","`app.startup`",[],[7,15,16,17,21,29,30,32,37],"Fixed startup ordering: config, logging, connection, migrations, strategies, session, order recovery, reconciliation, halt state, ready alert. 70% coverage."),
 (33,"app.loops","zarabot/app/loops.py","`app.loops` / `app.shutdown`",[],[1,2,8,"9b",20,21,33],"The trading cycle. Exits run before the halt check, which is what implements halt-blocks-entries-only. 70% coverage."),
